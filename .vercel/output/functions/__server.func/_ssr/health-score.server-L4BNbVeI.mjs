...
async function fetchSnapshot(supabase, userId) {
  ...
  const { data: recentTxns } = await supabase.from("transactions").select("date,merchant_name,category,type,amount").order("date", { ascending: false }).limit(30);
  return {
...
function computeBaseline(snapshot) {
  ...
  const savings_score = clamp(savingsRate * 4);
  ...
  const emergency_score = clamp(monthsCovered * 20);
...
async function scoreWithGemini(snapshot) {
...
async function computeAndStoreHealthScore(supabase, userId) {
